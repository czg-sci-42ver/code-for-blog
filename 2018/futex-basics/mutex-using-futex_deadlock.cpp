...
public:
  ...
  void lock() {
    ...
    if (c != 0) {
      ...
      #ifdef USE_DESIRE_ONE
      while ((c = cmpxchg(&atom_, 0, 1)) != 0);
      #else
  ...
  void unlock() {
    if (atom_.fetch_sub(1) != 1) {
      atom_.store(0);
      // std::cout << "store 0 to atom" << "\n";
      // std::cout << "FUTEX_WAKE_NUM: " << syscall(SYS_futex, (int*)&atom_, FUTEX_WAKE, 1, 0, 0, 0) << "\n";
      syscall(SYS_futex, (int*)&atom_, FUTEX_WAKE, 1, 0, 0, 0);
    }
    // std::cout << "when exiting unlock, atom:" << atom_ << "\n";
    // return -1;
  }

  void echo_atom_value(){
	std::cout << "atom:" << atom_ << "\n";
  }
